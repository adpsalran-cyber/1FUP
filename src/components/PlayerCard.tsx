...
interface PlayerCardProps {
  player: PlayerInput & {
    teamworkTendency?: 'Alto' | 'Medio' | 'Basso';
    gkEfficiency?: 'Alta' | 'Media' | 'Bassa';
  };
}
...
export const PlayerCard: React.FC<PlayerCardProps> = ({ player }) => {
  ...
  // Calcolo Gioco di Squadra (Alto / Medio / Basso)
  const teamwork = player.teamworkTendency || (
    (attrs.pas ?? 65) >= 75 ? 'Alto' : (attrs.pas ?? 65) >= 65 ? 'Medio' : 'Basso'
  );

  // Calcolo Efficacia da Portiere (Alta / Media / Bassa)
  const gkEff = player.gkEfficiency || (
    isGK
      ? 'Alta'
      : (attrs.rif ?? attrs.def ?? 60) >= 75
      ? 'Alta'
      : (attrs.rif ?? attrs.def ?? 60) >= 65
      ? 'Media'
      : 'Bassa'
  );

  const getStatusDot = (val: 'Alto' | 'Medio' | 'Basso' | 'Alta' | 'Media' | 'Bassa') => {
    switch (val) {
      case 'Alto':
      case 'Alta':
  ...
  return (
    <div className="relative w-full max-w-[280px] mx-auto rounded-2xl bg-gradient-to-b from-[#1a2333] via-[#121824] to-[#0b0e14] border-2 border-slate-700/60 p-4 shadow-xl">
      {/* Header Carta: Overall Bianco & Forma */}
      <div className="flex justify-between items-start mb-3">
        <div>
          <span className="font-bebas text-5xl font-extrabold text-white leading-none block drop-shadow-sm">
            {overall}
          </span>
        ...
        <div className="text-right">
          ...
          <span className={`font-bebas text-xl font-bold ${
            player.currentFormModifier > 0 ? 'text-emerald-400' : player.currentFormModifier < 0 ? 'text-rose-400' : 'text-slate-300'
          }`}>
      ...
      {/* Statistiche Principali in Italiano */}
        ...
        {isGK ? (
          <>
            ...
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              ...
              <span className="text-slate-100">{attrs.com ?? attrs.pre ?? 60}</span>
            </div>
          </>
        ) : (
          <>
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              ...
              <span className="text-slate-100">{attrs.pac ?? attrs.vel ?? 60}</span>
            </div>
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
            ...
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              ...
              <span className="text-slate-100">{attrs.sho ?? attrs.tir ?? 60}</span>
            </div>
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              <span className="text-slate-400">DIF</span>
              <span className="text-slate-100">{attrs.def ?? attrs.dif ?? 60}</span>
            </div>
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              <span className="text-slate-400">PAS</span>
              <span className="text-slate-100">{attrs.pas ?? 60}</span>
            ...
            <div className="flex justify-between border-b border-[#222c42]/60 pb-1">
              <span className="text-slate-400">FIS</span>
              <span className="text-slate-100">{attrs.phy ?? attrs.fis ?? 60}</span>
            </div>
          </>
        )}
      </div>

      {/* Indicatori Squadra & Portiere con Pallino Colorato */}
      <div className="mt-3 pt-2 border-t border-[#222c42] space-y-1.5 text-[11px]">
        <div className="flex items-center justify-between bg-[#0b0e14]/60 px-2 py-1 rounded-lg">
          <span className="text-slate-400 font-medium">Gioco di squadra:</span>
          <div className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full shadow-sm ${getStatusDot(teamwork)}`} />
            <span className="text-slate-200 font-bold">{teamwork}</span>
          </div>
        </div>

        <div className="flex items-center justify-between bg-[#0b0e14]/60 px-2 py-1 rounded-lg">
          <span className="text-slate-400 font-medium">Efficacia portiere:</span>
          <div className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full shadow-sm ${getStatusDot(gkEff)}`} />
            <span className="text-slate-200 font-bold">{gkEff}</span>
          </div>
        </div>
      </div>

      {/* Presenze / Vittorie / MVP */}
      <div className="mt-3 pt-2 border-t border-[#222c42] flex justify-around text-center text-[10px] text-slate-400 uppercase tracking-wider">
        <div>
          <span className="font-bebas text-sm text-slate-200 block">{player.matchesPlayed}</span>
          PRES
        </div>
        <div>
          <span className="font-bebas text-sm text-emerald-400 block">{player.wins}</span>
          VITT
        </div>
        <div>
